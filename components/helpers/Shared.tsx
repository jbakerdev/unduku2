...
export const ProgressBar = (value:number, max:number, tooltip?:string) => 
    <Tooltip placement="bottom" trigger={tooltip ? ['hover'] : []} overlay={<h6>{tooltip}</h6>}>
        <div style={{width:'100%', height:'100%', background:'transparent', border:'1px solid'}}>
            <div style={{transition:'width 500ms', background:colors.lGreen, width:Math.round((value/max)*100)+'%', height:'100%'}}/>
        </div>
    </Tooltip>
...
export const Icon = (iconBase64:string, tooltip:string, large?:boolean) => 
    <Tooltip placement="bottom" trigger={tooltip ? ['hover'] : []} overlay={<h6>{tooltip}</h6>}>
        <div style={{cursor:'pointer', width:large ? '32px' : '20px', height: large ? '32px' : '20px', backgroundImage:'url('+iconBase64+')', backgroundRepeat:'no-repeat', backgroundSize:'cover', backgroundPosition:"center", display:'inline-block'}}/>
    </Tooltip>
